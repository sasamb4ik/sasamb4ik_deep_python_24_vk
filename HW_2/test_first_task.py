import unittest
from first_task import process_json, remove_punctuation
...
class TestProcessJson(unittest.TestCase):
    ...
    def setUp(self):
        self.mocked_json_loads_return_value = {
            "gleb!": "This is a test!",
    ...
    def test_process_json_with_matches(self, mocked_json_loads):
        mocked_json_loads.return_value = self.mocked_json_loads_return_value

        tokens = ["this", "tests"]

        process_json(
            self.json_str, self.required_keys, tokens, self.mocked_callback
        )

        mocked_json_loads.assert_called_once_with(self.json_str)
